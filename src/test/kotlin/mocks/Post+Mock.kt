...
val Post.Companion.mockData: Post
    get() {
        ...
        resource?.let {
            ...
            val post = Json.create.decodeFromString<Post>(jsonContent)
            return post
        } ?: run {
...
val Post.Companion.mockDataWithNoEmbedded: Post
    get() {
        val classLoader = this::class.java.classLoader
        val resource = classLoader.getResource("PostNoEmbedded.json")
        resource?.let {
            val file = File(resource.file)
            val jsonContent = file.readText()
            val post = Json.create.decodeFromString<Post>(jsonContent)
            ...
            throw ErrorMock("PostNoEmbedded.json")
        }
    }
...
val Post.Companion.mockArrayData: List<Post>
    get() {
        ...
        resource?.let {
            val file = File(resource.file)
            val jsonContent = file.readText()
            val post = Json.create.decodeFromString<List<Post>>(jsonContent)
            return post
        } ?: run {
...
val Post.Companion.mockWithNoEmbedded: Post
    get() {
        val post = Post(
            id = 1234,
            date = LocalDateTime.parse("2023-10-06T14:08:00"),
            ...
            tags = listOf(3, 4),
            link = "https://example.com/1234",
            embeddedContent = EmbeddedContent.mockWithNoEmbedded
        )
        return post
    }
